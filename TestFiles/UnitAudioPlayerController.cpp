...
void UnitAudioPlayerController::handleKeyboardNormal()
{
    ...
    for (auto k : st.word)
    {
        ...
        case 'G':
            isInputMode = true;
            inputTrackNumber = "";
                        //mainOS->ShowOnScreenMessege("33",500);
...
void UnitAudioPlayerController::handleKeyboardInput()
{
    ...
    for (auto k : st.word)
    {
        if (k == 'g' || k == 'G')
        {
           // mainOS->ShowOnScreenMessege("22",500);
            isInputMode = false;
            inputTrackNumber = "";
            lastDiskPlaying = !g_audioState.isPlaying;
            drawMainScreen();
...
void UnitAudioPlayerController::drawVuMeter()
{
    ...
    for (int i = 0; i < 12; i++)
    {
        ...
        if (i < 6)
            on_c = AP_COLOR_CREAM2;
        else if (i < 9)
            on_c = AP_COLOR_AMBER;
        else if (i < 11)
            on_c = AP_COLOR_ORANGE;
        else
            on_c = AP_COLOR_RED;
...
void UnitAudioPlayerController::drawLeftPanel()
{
    ...
    unsigned long el = isPlaying
                           ? (millis() - g_audioState.trackStartTime - g_audioState.totalPausedTime) / 1000
                           : 0;
...
{
    if (g_audioState.currentVolume >= 30)
        return;
...
{
    if (g_audioState.currentVolume <= 0)
        return;
    g_audioState.currentVolume--;
    g_audioPlayer.setVolume(g_audioState.currentVolume);
    char m[20];
...
void UnitAudioPlayerController::checkAutoNext()
{
    ...
    if (g_audioState.lastPlayStatus == AUDIO_PLAYER_STATUS_PLAYING && cs == AUDIO_PLAYER_STATUS_STOPPED)
    {
        if (g_audioState.isLoopEnabled)
        {
            delay(100);
            ...
            g_audioState.isPlaying = true;
            if (!isInfoScreen)
                showTempMsg("LOOP", AP_COLOR_AMBER);
        }
        else if (g_audioState.isShuffleEnabled)
        {
            uint16_t r = random(1, g_audioState.totalTracks + 1);
            g_audioPlayer.selectAudioNum(r);
            delay(200);
            g_audioPlayer.playAudio();
            delay(100);
            g_audioState.trackStartTime = millis();
            ...
            g_audioState.isPlaying = true;
            if (!isInfoScreen)
                showTempMsg("SHUFFLE", AP_COLOR_WARM);
        }
        else
        {
            g_audioPlayer.nextAudio();
            ...
            g_audioState.isPlaying = true;
            if (!isInfoScreen)
                showTempMsg("AUTO NEXT", AP_COLOR_CREAM1);
        }
    }
    ...
    if (g_audioState.lastPlayStatus != cs)
    {
        ...
        {
            if (g_audioState.lastPlayStatus == AUDIO_PLAYER_STATUS_PAUSED)
                g_audioState.totalPausedTime +=
                    (millis() - g_audioState.pausedTime);
            g_audioState.isPlaying = true;
        }
